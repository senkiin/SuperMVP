    ...
    @if ($activeConversation)
        <div class="flex-1 flex flex-col bg-white dark:bg-gray-900">
            <div class="p-4 border-b dark:border-gray-700 flex justify-between items-center shrink-0">
                <h2 class="font-semibold text-lg text-gray-800 dark:text-gray-200 truncate pr-4">{{ $activeConversation->title }}</h2>
            ...
            <div x-ref="chat" class="flex-1 p-6 space-y-6 overflow-y-auto">
                @foreach ($messages as $message)
                    <div class="flex items-start gap-3 @if($message['sender'] == 'user') flex-row-reverse @endif">
                        ...
                        @endif
                        <div class="max-w-2xl px-4 py-3 rounded-lg @if($message['sender'] == 'user') bg-indigo-600 text-white rounded-br-none @else bg-gray-100 dark:bg-gray-700 text-gray-900 dark:text-gray-200 rounded-bl-none @endif">
            ...
            <div class="p-4 bg-white dark:bg-gray-900/80 border-t dark:border-gray-700 shrink-0">
                <div class="max-w-3xl mx-auto">
                    <form wire:submit="sendMessage" class="flex items-center space-x-3">
                        ...
                        <button type="submit" class="inline-flex items-center justify-center size-10 bg-indigo-600 text-white rounded-full hover:bg-indigo-700 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-indigo-500 disabled:opacity-50 transition-colors shrink-0" wire:loading.attr="disabled">
                            <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 20 20" fill="currentColor" class="w-5 h-5"><path d="M3.105 2.289a.75.75 0 00-.826.946l1.414 4.949a.75.75 0 00.926.606a11.902 11.902 0 012.243-.335.75.75 0 00.505-.654l.28-1.033a.75.75 0 00-.34-.838A11.953 11.953 0 013.105 2.29zM4.493 8.544a11.899 11.899 0 01-2.42 1.65a.75.75 0 00-.364 1.118l7.133 9.512a.75.75 0 001.248-.032l4.23-7.922a.75.75 0 00-.654-.926a11.902 11.902 0 01-3.23.454a.75.75 0 00-.654.505l-1.033.28a.75.75 0 00-.838-.34a11.953 11.953 0 01-2.29-3.105z" /></svg>
                        </button>
                    </form>
        ...
        <div x-show="showSettings" 
             ...
             class="w-96 bg-white dark:bg-gray-800 border-l dark:border-gray-700 p-6 flex flex-col space-y-6 overflow-y-auto absolute top-0 right-0 h-full z-10 shadow-lg"
