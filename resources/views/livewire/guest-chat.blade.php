...
<div x-data="{
    ...
    <div
        class="absolute inset-0 bg-gradient-to-br from-indigo-50/30 to-purple-50/30 dark:from-indigo-900/10 dark:to-purple-900/10 opacity-60">
    </div>
    <div
        class="absolute inset-0 bg-[radial-gradient(circle_at_25%_25%,rgba(99,102,241,0.1),transparent_50%)] dark:bg-[radial-gradient(circle_at_25%_25%,rgba(99,102,241,0.05),transparent_50%)]">
    </div>

    {{-- Header elegante para invitados --}}
    <div
        class="relative z-10 h-20 px-8 py-4 border-b border-gray-200/50 dark:border-gray-700/50 bg-white/80 dark:bg-gray-900/80 backdrop-blur-xl">
        <div class="flex justify-between items-center h-full">
            <div class="flex items-center space-x-4">
                <div
                    class="w-12 h-12 bg-gradient-to-br from-indigo-500 to-purple-600 rounded-2xl flex items-center justify-center shadow-lg shadow-indigo-500/25">
                ...
                <span
                    class="inline-flex items-center px-3 py-1 rounded-full text-xs font-medium bg-gradient-to-r from-blue-100 to-indigo-100 dark:from-blue-900/30 dark:to-indigo-900/30 text-blue-800 dark:text-blue-300 border border-blue-200 dark:border-blue-700">
    ...
    <div x-ref="chat"
        class="flex-1 overflow-y-auto scrollbar-thin scrollbar-thumb-indigo-300 dark:scrollbar-thumb-indigo-700 scrollbar-track-transparent relative z-10">
        <div class="px-8 py-8 space-y-8 max-w-6xl mx-auto" style="padding-bottom: 160px;">
            {{-- Mensaje de bienvenida para invitados --}}
            @if (empty($messages))
                <div class="text-center py-16">
                    <div
                        class="w-20 h-20 bg-gradient-to-br from-indigo-100 to-purple-100 dark:from-indigo-900/30 dark:to-purple-900/30 rounded-full flex items-center justify-center mx-auto mb-6">
                    ...
                    <p class="text-gray-600 dark:text-gray-400 mb-6 max-w-md mx-auto">Ask me anything! I'm here to help
                        you with questions, tasks, and conversations.</p>
                    ...
                    <div
                        class="bg-gradient-to-r from-indigo-500/10 to-purple-500/10 dark:from-indigo-900/20 dark:to-purple-900/20 rounded-2xl p-6 border border-indigo-200/50 dark:border-indigo-700/50">
                        <h3 class="text-lg font-semibold text-gray-900 dark:text-white mb-2">💡 Want More Features?</h3>
                        <p class="text-gray-600 dark:text-gray-400 text-sm mb-4">Sign up to save your conversations,
                            access advanced settings, and unlock premium features!</p>
            ...
            @foreach ($messages as $message)
                <div class="flex items-start gap-6 @if ($message['sender'] == 'user') flex-row-reverse @endif">
                    ...
                    @if ($message['sender'] == 'ai')
                        <div
                            class="flex items-center justify-center w-12 h-12 bg-gradient-to-br from-indigo-500 to-purple-600 rounded-2xl text-white font-bold text-sm shadow-lg shadow-indigo-500/25 flex-shrink-0">
                    ...
                    @else
                        <div
                            class="flex items-center justify-center w-12 h-12 bg-gradient-to-br from-gray-600 to-gray-700 rounded-2xl text-white font-bold text-sm shadow-lg shadow-gray-500/25 flex-shrink-0">
                            <svg class="w-6 h-6" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
            ...
            @if ($loading)
                <div class="flex items-start gap-6">
                    ...
                    <div class="flex-1 max-w-4xl">
                        <div
                            class="bg-white/80 dark:bg-gray-800/80 rounded-3xl rounded-bl-lg shadow-lg shadow-gray-500/10 dark:shadow-gray-900/20 backdrop-blur-sm border border-gray-200/50 dark:border-gray-700/50 px-6 py-4">
                            <div class="flex items-center space-x-3">
    ...
    <div
        class="absolute bottom-0 left-0 right-0 z-20 p-6 bg-white/80 dark:bg-gray-900/80 backdrop-blur-xl border-t border-gray-200/50 dark:border-gray-700/50">
        <div class="max-w-6xl mx-auto">
            <form wire:submit="sendMessage" class="relative">
                <div class="flex items-end space-x-4">
                    <div class="flex-1 relative">
                        <textarea wire:model="newMessage" placeholder="Ask me anything..."
                            ...
                            rows="1" style="min-height: 56px; max-height: 120px;" autocomplete="off" wire:loading.attr="disabled"
                            x-data="{
                                resize: function() {
                                    this.$el.style.height = 'auto';
                                    this.$el.style.height = Math.min(this.$el.scrollHeight, 120) + 'px';
                                }
                            }" x-on:input="resize" x-init="resize"></textarea>
            ...
            <div
                class="flex flex-col sm:flex-row justify-between items-start sm:items-center mt-4 space-y-2 sm:space-y-0">
                <div class="flex items-center space-x-2 text-xs text-gray-500 dark:text-gray-400">
                    <svg class="w-4 h-4 text-yellow-500" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                            d="M12 9v2m0 4h.01m-6.938 4h13.856c1.54 0 2.502-1.667 1.732-2.5L13.732 4c-.77-.833-1.964-.833-2.732 0L3.238 16.5c-.77.833.192 2.5 1.732 2.5z">
                        </path>
                    </svg>
    ...
    <script>
        let activePolls = {};
        let pollCount = 0;
        const MAX_RETRIES = 110; // 60 segundos máximo

        document.addEventListener('livewire:initialized', () => {
            console.log('✅ Livewire chat initialized');

            // Escuchar eventos de polling
            Livewire.on('start-polling', (data) => {
                console.log('🚀 Starting polling for message:', data.messageId);
                startPolling(data.messageId);
            });

            Livewire.on('stop-polling', (data) => {
                console.log('⏹️ Stopping polling for message:', data.messageId);
                stopPolling(data.messageId);
            });
        });

        function startPolling(messageId) {
            // Prevenir múltiples polling para el mismo mensaje
            if (activePolls[messageId]) {
                console.log('⚠️ Polling already active for:', messageId);
                return;
            }

            console.log('🔄 Polling started for:', messageId);
            pollCount = 0;

            // Hacer polling cada 2 segundos
            activePolls[messageId] = setInterval(() => {
                pollCount++;

                // Detener después de MAX_RETRIES intentos
                ...
                // Solo buscar por messageId específico primero
                @this.checkForResponse(messageId).then((hasResponse) => {
                    if (hasResponse) {
                        console.log('✅ Response received for:', messageId);
                        stopPolling(messageId);
                    }
                }).catch((error) => {
                ...
                // Si no encuentra con messageId, intentar sin él cada 3 intentos
                if (pollCount % 3 === 0) {
                    @this.checkForResponse(null).then((hasResponse) => {
                        if (hasResponse) {
        ...
        function stopPolling(messageId) {
            if (activePolls[messageId]) {
                clearInterval(activePolls[messageId]);
                delete activePolls[messageId];
                pollCount = 0;
                console.log('⏹️ Polling stopped for:', messageId);
            }
        }

        // Cleanup al salir de la página
        window.addEventListener('beforeunload', () => {
            Object.keys(activePolls).forEach(messageId => {
                stopPolling(messageId);
            });
        });

        // Debug: Mostrar estado de polling cada 10 segundos
        setInterval(() => {
            const activeCount = Object.keys(activePolls).length;
            if (activeCount > 0) {
                console.log(`📊 Polling status: ${activeCount} active polls, attempt ${pollCount}/${MAX_RETRIES}`);
            }
        }, 10000);
    </script>
    {{-- Estilos adicionales --}}
